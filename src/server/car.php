...
if($type == "add")
{
  ...
  $sql = "SELECT cart.*,goods.title,goods.img,goods.price FROM cart, goods WHERE cart.good_id = goods.id AND cart.id=$id";
//   $sql = "SELECT cart.*,goods.title,goods.img,goods.price FROM cart, goods WHERE cart.good_id = goods.id AND id=$id";
  $a=mysqli_query($db, $sql);
  $data = mysqli_fetch_all($a,MYSQLI_ASSOC);
//   print_r($data);
  /* 遍历，根据数据来调整结构 */
